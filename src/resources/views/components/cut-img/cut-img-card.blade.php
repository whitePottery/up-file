@foreach($images as $image)
  <div class="img-item" id="img_{{ $image->id }}" data-id="{{ $image->id }}">
    ...
    <a herf="#" class="img-croping {{ $image->src_cut?'img-green':'img-black' }}" onclick="imgCroping('img_{{ $image->id }}');" title="Сделать превью для картинки" onmouseover="viewCutImage({{ $image->id }})"></a>
