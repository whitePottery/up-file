...
class CutImg extends Component
{
    ...
    private function cardCreate()
    {
        ...
        $data = UploadImage::select('id', 'url_img', 'path_mini', 'name_img', 'ext_img', 'post_id', 'user_id', 'alt')->where('name_model', $this->nameModel)->where('post_id', $this->postId)->where('user_id', $this->user_id)->get();
